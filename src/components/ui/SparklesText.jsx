...
function SparkleIcon({ id, x, y, color, delay, scale }) {
  return (
    <motion.svg
      key={id}
      className="sparkle-svg"
      initial={{ left: x, top: y, opacity: 0.7 }}
      animate={{
        y: [0, -6, 0, 4, 0],
        x: [0, 3, -2, 1, 0],
        rotate: [0, 15, -10, 5, 0],
        scale: [scale, scale * 1.15, scale * 0.9, scale * 1.05, scale]
      }}
      transition={{
        duration: 4 + delay,
        repeat: Infinity,
...
export default function SparklesText({
  ...
  return (
    <div
      className={`sparkles-text ${className}`.trim()}
      style={{
