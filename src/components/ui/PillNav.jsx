...
const PillNav = ({
  ...
  useEffect(() => {
    ...
    if (initialLoadAnimation) {
      ...
      if (navEl) { gsap.set(navEl, { width: 0, overflow: 'hidden' }); gsap.to(navEl, { width: 'auto', duration: 0.6, ease }); }
    }
      ...
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        <Link className="pill-logo" to="/" aria-label="Home" onMouseEnter={handleLogoEnter}
          ...
          {logo
            ...
            : <span ref={logoImgRef} style={{ fontSize: '1.25rem' }}>📚</span>}
        </Link>
        ...
        <button className="mobile-menu-button mobile-only" onClick={toggleMobileMenu}
          aria-label="Toggle menu" ref={hamburgerRef}>
